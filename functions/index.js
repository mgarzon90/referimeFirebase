...
const gmailEmail = encodeURIComponent('kami.genius@gmail.com');
const gmailPassword = encodeURIComponent('@amna2014');
  ...
  return Promise.all([getDeviceTokensPromise, getSenderProfilePromise]).then(results => {
    ...
    return admin.messaging().sendToDevice(tokensSnapshot.val(), payload).then(response => {
      ...
      referralCheck = admin.database().ref(`referral/${friendUid}/${userUid}`).once('value').then(snap => {
        ...
        refferedPoints = admin.database().ref(`/accounts/${refferedUid}/referral-points`).once('value').then(snap => {
          console.log('referral point', snap.val())
          return admin.database().ref(`/accounts/${refferedUid}/referral-points`).set(snap.val() + 1);
        });
        let obj = snap.val();
...
exports.sendEmail = functions.database.ref('/emails/{emailId}').onWrite(event => {
  let request = event.data.val();
  const mailOptions = {
    from: '"Referral Race" <referralrace@algorithmous.com>',
    to: request.to
  };

  mailOptions.subject = request.subject;
  mailOptions.text = request.html;
